    ...
    <div className="flex flex-col gap-4 animate-fade-in">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 rounded-md border border-border bg-muted hover:bg-secondary text-sm text-foreground transition-colors"
        >
      ...
      {showForm && (
        <div className="bg-card border border-primary/30 rounded-lg p-4 animate-fade-in glow-blue">
          ...
          <div className="flex gap-2 mt-3">
            <button onClick={handleAdd} className="px-4 py-2 rounded bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors">
              Сохранить
            </button>
            <button onClick={() => setShowForm(false)} className="px-4 py-2 rounded bg-muted border border-border text-sm text-foreground hover:bg-secondary transition-colors">
              Отмена
            </button>
          </div>
        </div>
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={8} className="text-center py-12 text-muted-foreground text-sm">Ничего не найдено</td></tr>
              )}
              {filtered.map(c => {
                ...
                return (
                  <tr
                    key={c.id}
                    className={`border-b border-border row-hover cursor-pointer ${isSelected ? "bg-primary/8 border-l-2 border-l-primary" : ""}`}
                    onClick={() => onSelect(isSelected ? null : c.id)}
                  >
                    <td className="px-4 py-3 font-mono-custom font-medium text-foreground">{c.name}</td>
                    <td className="px-4 py-3 font-mono-custom text-muted-foreground text-xs">{c.inventory}</td>
                    <td className="px-4 py-3 text-muted-foreground">{c.model}</td>
                    <td className="px-4 py-3 text-muted-foreground">{c.location}</td>
                    <td className="px-4 py-3 text-foreground">{c.user}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-medium ${STATUS_COLORS[c.status]}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${STATUS_DOT[c.status]}`} />
                        {c.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 font-mono-custom text-xs text-muted-foreground">{c.ip}</td>
                    <td className="px-4 py-3">
                      {docCount > 0 ? (
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {!filterComputerId && (
          <FilterSelect
            ...
            options={computers.map(c => c.id)}
            label="Все компьютеры"
          />
        )}
        <button
          onClick={handleExport}
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="text-center py-12 text-muted-foreground text-sm">Ничего не найдено</td></tr>
              )}
              {filtered.map(d => {
                const comp = computers.find(c => c.id === d.computerId);
                return (
                  <tr key={d.id} className="border-b border-border row-hover">
                    <td className="px-4 py-3 text-foreground font-medium">{d.title}</td>
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded text-xs bg-accent text-accent-foreground">{d.type}</span>
                    </td>
                    <td className="px-4 py-3 font-mono-custom text-xs text-muted-foreground">{d.number}</td>
                    <td className="px-4 py-3">
                      {comp ? (
                    ...
                    <td className="px-4 py-3 font-mono-custom text-xs text-muted-foreground">{d.date}</td>
                    <td className="px-4 py-3 text-muted-foreground text-xs max-w-48 truncate">{d.note || "—"}</td>
                  </tr>
                );
              })}
            </tbody>
